...
它采用 local-first 设计：消息路由、桌面控制台、Route 状态和文件缓存都运行在你的
Mac 上。每种工具集成都做成独立的 **Route**，可以单独安装或移除，不需要把所有功能
都塞进主程序。
...
- 和默认助手聊天，或随时切换到更专门的 Route。
- 在微信与支持的本地工具之间传递文字、图片和文件。
- 在一个桌面应用里查看连接状态、Routes、配置和运行记录。
- 从 Community 浏览和管理独立发布的 Routes。
...
flowchart LR
  ...
  C --> D["macOS 控制台<br/>连接、配置、查看状态"]
  P --> W
```

WeConnect 本体负责微信连接、消息标准化、本地状态、Route 切换和桌面控制台。每个
Route 只负责自己的工具逻辑和专属依赖。
