...
This package is the bottom protocol layer. It owns only WeChat iLink mechanics:
QR login, credentials in memory, long polling, context tokens, media CDN
...
- TypeScript.
- Node.js built-ins only at runtime.
- `tsdown` for builds.
- Node test runner for tests.
- Optional `qrcode-terminal` only for the example bot.
